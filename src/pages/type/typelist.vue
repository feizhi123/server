                ...
                <div class="block-header form-horizontal">
                    <div class="row">
                        <div class="col-md-1 fl">
                            <router-link class="btn btn-sm btn-primary fl dll-add" :to="['/detail/add_detail?type=']+typeid">新增信息</router-link>
                        </div>
    ...
    export default {
        ...
        mounted() {
            if(this.$route.query.title!=""&&typeof this.$route.query.title!='undefined'){
                ...
                this.getdata();
                detailApi.getCountList(this.typeid).then(data => {
        ...
        methods: {
            ...
            getDetail(){
                this.$router.push({path:"/typelist",query:{type:this.typeid,page:this.queryParams.page,name:this.title}})
            },
            getdata(){
                var params={
                    type:this.typeid,
            ...
            doConfirm(){
                ...
                detailApi.deleteDetail(params).then(data => {
                    ...
                    if(data.code==0)
                    {
                        ...
                        this.getDetailbyname(this.titleLabel.searchTitle)
            ...
            pageChange(page) {
                ...
                if(typeof vm.$route.query.title!='undefined'){
                    vm.$router.push({path:"typelist",query:{type:vm.typeid,title:vm.$route.query.title,page:page,name:this.title}})
                }
                else{
                    vm.$router.push({path:"typelist",query:{type:vm.typeid,page:page,name:this.title}})
                }
            ...
            search() {
                ...
                if(vm.queryParams.queryStr!=''){
                    vm.$router.push({path:"typelist",query:{type:vm.typeid,title:vm.queryParams.queryStr,page:vm.queryParams.page,name:this.title}})
                }
                else
               vm.$router.push({path:"typelist",query:{type:vm.typeid,page:vm.queryParams.page,name:this.title}})
                vm.queryParams.queryStr=''
            },
            getDetailbyname(title){
                var params={
                    type:this.typeid,
                    title:title,
                    perpage:this.queryParams.perPage,
